...
# --- FFmpeg Auto-Integration (UPDATED) ---
try:
    import static_ffmpeg

    # This forces the bundled ffmpeg/ffprobe into the system's temporary PATH
    static_ffmpeg.add_paths()
    FFMPEG_AVAILABLE = True
except ImportError:
    FFMPEG_AVAILABLE = False

ssl._create_default_https_context = ssl._create_unverified_context  # mac_SSL byPass
...
class App:
    def __init__(self, root):
        ...
        theme_toggle = ttk.Checkbutton(button_frame, text="Dark Mode", variable=self.dark_mode,
        ...
        # Alert the user if FFmpeg isn't found
        if not FFMPEG_AVAILABLE:
            self.update_status("Warning: static_ffmpeg missing. HD Audio merging may fail.")
    ...
    def open_download_folder(self):
        ...
        try:
            ...
            # os.startfile is Windows only. This fallback handles Mac/Linux.
    ...
    def format_seconds(s):
        if s is None: return "--:--"
        try:
            m, s = divmod(int(s), 60); h, m = divmod(m, 60); return f"{h:d}:{m:02d}:{s:02d}" if h else f"{m:d}:{s:02d}"
        except Exception:
            return "--:--"
        ...
        try:
            ...
            for f in info.get('formats', []):
                if f.get('vcodec', 'none') != 'none':
                    ...
                    if filesize_mb:
                        filesize_str = f"{filesize_mb / (1024 * 1024):.1f} MB".rjust(10)
                    else:
                        filesize_str = "Unknown MB".rjust(10)
    ...
    def progress_hook(self, d):
        if d['status'] == 'downloading':
            ...
            self.root.after(0, self._update_progress_display, 100, "Download complete, processing and merging file...")
        elif d['status'] == 'error':
